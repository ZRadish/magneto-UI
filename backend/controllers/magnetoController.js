...
export const runLanguageDetection = (req, res) => {
    ...
    const scriptPath = path.join('/app/magneto/languageDetection', 'detectLanguageAll.py');
    const scriptArgs = ['-a', argA, '-b', argB];

    // Spawn the Python process using Poetry
    const pythonProcess = spawn('/root/.local/bin/poetry', ['run', 'python', scriptPath, ...scriptArgs], {
        cwd: '/app/magneto/languageDetection', // Correct working directory
        env: { ...process.env, PYTHONPATH: '/app/magneto' },
    });
...
export const runUserEnteredData = (req, res) => {
    ...
    const scriptPath = path.join('/app/magneto/userEnteredData', 'findTriggerCheckInput.py');
    const scriptArgs = ['-a', argA, '-b', argB];

    // Spawn the Python process using Poetry
    const pythonProcess = spawn('/root/.local/bin/poetry', ['run', 'python', scriptPath, ...scriptArgs], {
        cwd: '/app/magneto/userEnteredData', // Correct working directory
        env: { ...process.env, PYTHONPATH: '/app/magneto' },
    });
