...
# Add `/app/magneto` to the Python path
scriptLocation = os.path.dirname(os.path.abspath(__file__))  # Get the current script directory
parentDir = os.path.abspath(os.path.join(scriptLocation, ".."))  # Get the parent directory
sys.path.insert(0, parentDir)  # Add parent directory to the Python path
...
def find_xml_from_screenshot(imagename, stepNum, args):
   #print(f"Processing screenshot: {imagename}")
    ...
    #print(f"Generated XML name: {fullpath}")
    return fullpath
...
def find_xml_crashscope(imagename, stepNum, args):
    ...
    #print index 0, 1, 2 of dot_split_parts
    # print(f"dot_split_parts: {dot_split_parts}")
    # print(f"underscore_split_parts: {underscore_split_parts}")
